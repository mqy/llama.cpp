...
struct ggml_compute_state_2 {
    struct ggml_compute_params params;
    struct ggml_tensor * node;
};

struct ggml_compute_state_shared_2 {
    // spin lock.
    atomic_flag spin;
    ...
    struct ggml_compute_state_2 tasks[GGML_MAX_N_THREADS];

    // thread ids.
    pthread_t thread_ids[GGML_MAX_N_THREADS];
};

static thread_ret_t ggml_graph_compute_thread_2(void * args) {
    struct ggml_compute_state_shared_2 * shared = (struct ggml_compute_state_shared_2 *)args;
    struct ggml_compute_state_2 * task = NULL;
...
static void ggml_graph_compute_plan(struct ggml_context * ctx, struct ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            node->mul_mat_use_blas = false;
            node->task_stages[j].n_tasks = 0;
            node->task_stages[j].worker_wait = false;
        }
        ...
        switch (node->op) {
            ...
            case GGML_OP_DUP:
                {
                    ...
                    node->task_stages[GGML_TASK_COMPUTE].n_tasks = n_threads;
            ...
            case GGML_OP_RELU:
                {
                    node->task_stages[GGML_TASK_COMPUTE].n_tasks = 1;
                } break;
            case GGML_OP_MUL:
            case GGML_OP_GELU:
            case GGML_OP_SILU:
            ...
            case GGML_OP_MUL_MAT:
                {
                    ...
                    if (ggml_is_contiguous(node->src0) && ggml_is_contiguous(node->src1)) {
                        ...
                        if (M >= 32 && N >= 32 && K >= 32) {
                            node->mul_mat_use_blas = true;
                        }

                        if (cgraph->mulmat_bench) {
                            const char *blas_name = ggml_get_blas_name();
                            if (blas_name != NULL && strcmp(blas_name, cgraph->mulmat_bench->blas_name) == 0) {
                                node->mul_mat_use_blas = ggml_mulmat_bench_use_blas(cgraph->mulmat_bench, M, N, K, n_threads);
                            }
                        }
                    }

                    if (node->mul_mat_use_blas) {
                        node->task_stages[GGML_TASK_COMPUTE].n_tasks = 1;
                        node->task_stages[GGML_TASK_COMPUTE].worker_wait = true;
                    } else {
                        node->task_stages[GGML_TASK_COMPUTE].n_tasks = n_threads;
                    }
                    ...
                    if (node->src0->type == GGML_TYPE_F16 && node->src1->type == GGML_TYPE_F32) {
                        ...
                        if (node->mul_mat_use_blas) {
                            ...
                            cur = ggml_cuda_mul_mat_get_wsize(node->src0, node->src1, node);
#elif defined(GGML_USE_ACCELERATE) || defined(GGML_USE_OPENBLAS) 
                            node->task_stages[GGML_TASK_INIT].n_tasks = n_threads;
#endif
                            cur = GGML_TYPE_SIZE[GGML_TYPE_F32]*(node->src0->ne[0]*node->src0->ne[1]);
                        } else {
            ...
            case GGML_OP_ALIBI:
                {
                    node->task_stages[GGML_TASK_COMPUTE].n_tasks = 1; //TODO
                } break;
            case GGML_OP_CONV_1D_1S:
            case GGML_OP_CONV_1D_2S:
                {
                    node->task_stages[GGML_TASK_INIT].n_tasks = 1;
                    node->task_stages[GGML_TASK_COMPUTE].n_tasks = n_threads;
...
void ggml_graph_compute(struct ggml_context * ctx, struct ggml_cgraph * cgraph) {
    ...
    struct ggml_compute_state_shared_2 * state_shared = NULL;

    // create thread pool.
    if (n_threads > 1) {
        { // state_shared
            int64_t len = sizeof(struct ggml_compute_state_shared_2);
        ...
        for (int j = 0; j < n_threads - 1; j++) {
            int rc = ggml_thread_create(&state_shared->thread_ids[j], NULL, ggml_graph_compute_thread_2, state_shared);
            GGML_ASSERT(rc == 0);
            UNUSED(rc);
        }
    }
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        for (int k = GGML_TASK_INIT; k <= GGML_TASK_FINALIZE; k++) {
            ...
            struct ggml_compute_task_stage *tc = &node->task_stages[k];
            if (tc->n_tasks < 1) {
                continue;
            }
            ...
            if (n_worker_tasks > 0) {
                ...
                for (int j = 0; j < n_worker_tasks; j++) {
                    state_shared->tasks[j] = (struct ggml_compute_state_2) {
                        .params = {
                            .type  = type,
                            .ith   = j + 1,
                            .nth   = tc->n_tasks,
