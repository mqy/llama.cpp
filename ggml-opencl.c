...
const char *clblast_dequant = MULTILINE_QUOTE(
    ...
    typedef uchar uint8_t; typedef int int32_t; typedef uint uint32_t;

    constant uint QK4_0 = 32; struct block_q4_0 {
        float d;
        uint8_t qs[32 / 2]; // 32: QK4_0
    };

    constant uint QK4_1 = 32; struct block_q4_1 {
        float d;
        float m;
        uint8_t qs[32 / 2]; // 32: QK4_1
    };

    constant uint QK5_0 = 32; struct __attribute__((packed)) block_q5_0 {
        half d;
        uint32_t qh;
    ...
    constant uint QK8_0 = 32; struct block_q8_0 {
        float d;
        uint8_t qs[32]; // 32: QK8_0
    };

    __kernel void dequantize_row_q4_0(__global struct block_q4_0 *x,
                                      __global float *y) {
        uint qk = QK4_0;
    ...
    __kernel void dequantize_row_q4_1(__global struct block_q4_1 *x,
                                      __global float *y) {
        uint qk = QK4_1;
    ...
    __kernel void dequantize_row_q5_0(__global struct block_q5_0 *x,
                                      __global float *y) {
        uint qk = QK5_0;
    ...
    __kernel void dequantize_row_q5_1(__global struct block_q5_1 *x,
                                      __global float *y) {
        uint qk = QK5_1;
    ...
    __kernel void dequantize_row_q8_0(__global struct block_q8_0 *x,
                                      __global float *y) {
        uint qk = QK8_0;
        const uint i = get_global_id(0) / qk;
        const uint j = get_local_id(0);
...
static cl_kernel kernel_q4_0, kernel_q4_1, kernel_q5_0, kernel_q5_1,
    kernel_q8_0;
static cl_mem cl_buffer_a = NULL, cl_buffer_qb = NULL, cl_buffer_b = NULL,
              cl_buffer_c = NULL;
static size_t cl_size_a = 0, cl_size_qb = 0, cl_size_b = 0, cl_size_c = 0;

static size_t device_global_mem_size;
static size_t device_max_mem_alloc_size;
...
void ggml_cl_init(void) {
    ...
    printf("\nAttempting to use: Platform=%d Device=%d (If invalid, program "
           "will crash)\n",
           plat_num, dev_num);
...
static void ggml_cl_malloc(size_t req_size, size_t *cur_size,
                           cl_mem_flags flags, cl_mem *buf) {
...
static int ggml_cl_num_pass(size_t size_a, size_t size_b, size_t size_c,
                            size_t size_qb) {
...
void ggml_cl_sgemm_wrapper(const enum ggml_blas_order order,
                           const enum ggml_blas_op trans_a,
                           const enum ggml_blas_op trans_b, const int m,
                           const int n, const int k, const float alpha,
                           const void *host_a, const int lda,
    ...
    case GGML_TYPE_Q5_0:
        ...
        local = 16;
        size_qb =
            global * (sizeof(ggml_fp16_t) + sizeof(uint32_t) + local) / 32;
        break;
    case GGML_TYPE_Q5_1:
        dequant = true;
        kernel = kernel_q5_1;
        local = 16;
        size_qb =
            global * (sizeof(ggml_fp16_t) * 2 + sizeof(uint32_t) + local) / 32;
        break;
    case GGML_TYPE_Q8_0:
        dequant = true;
        kernel = kernel_q8_0;
    ...
    if (device_global_mem_size > 0 || device_max_mem_alloc_size > 0) {
        num_pass =
            ggml_cl_num_pass(size_a, size_b, size_c, dequant ? size_qb : 0);
    }
    ...
    for (int pass = 0; pass < num_pass; pass++) {
        if (dequant) {
            err = clEnqueueWriteBuffer(
                queue, cl_buffer_qb, is_blocking_queue ? CL_TRUE : CL_FALSE, 0,
                size_qb, host_b + (pass * size_qb), 0, NULL, &ev_qb);
            ...
            err = clEnqueueWriteBuffer(
                queue, cl_buffer_b, is_blocking_queue ? CL_TRUE : CL_FALSE, 0,
                size_b, host_b + (pass * size_b), 0, NULL, &ev_b);
            CL_CHECK(err, "clEnqueueWriteBuffer b");
        }

        err = clEnqueueWriteBuffer(queue, cl_buffer_a,
                                   is_blocking_queue ? CL_TRUE : CL_FALSE, 0,
                                   size_a, host_a, 0, NULL, &ev_a);
        ...
        CLBlastStatusCode status = CLBlastSgemm(
            (CLBlastLayout)order, (CLBlastTranspose)trans_a,
            (CLBlastTranspose)trans_b, m, N, k, alpha, cl_buffer_a, 0, lda,
            cl_buffer_b, 0, ldb, beta, cl_buffer_c, 0, LDC, &queue, &ev_sgemm);
        ...
        cl_event ev_c;
        clEnqueueReadBuffer(queue, cl_buffer_c, CL_TRUE, 0, size_c, temp_host_c,
                            1, &ev_sgemm, &ev_c);
